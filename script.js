...
// 新規実践開始モーダル制御
let modalSessionType = 'new';

function selectModalSessionType(type) {
    modalSessionType = type;
    document.getElementById('modalTypeNew').className = 'select-btn ' + (type === 'new' ? 'sel-active' : '');
    document.getElementById('modalTypeMid').className = 'select-btn ' + (type === 'mid' ? 'sel-active' : '');
}
...
// 店舗情報＆日付編集モーダル制御
let editSessionReturnToList = false;
let editModalSessionType = 'new';

// 稼働モード（新規=稼働用 / mid=後ヅモ用）を判定。旧データは開始G数・前任者情報の有無から推定する
function getEffectiveMode(t) {
    if (!t) return 'new';
    if (t.mode === 'new' || t.mode === 'mid') return t.mode;
    if ((t.startGames || 0) > 0 || (t.prevBig || 0) > 0 || (t.prevReg || 0) > 0) return 'mid';
    return 'new';
}

function selectEditSessionType(type) {
    editModalSessionType = type;
    document.getElementById('editTypeNew').className = 'select-btn ' + (type === 'new' ? 'sel-active' : '');
    document.getElementById('editTypeMid').className = 'select-btn ' + (type === 'mid' ? 'sel-active' : '');
}

function openEditSessionModal(returnToList) {
    editSessionReturnToList = !!returnToList;
    ...
    selectEditSessionType(getEffectiveMode(t));
...
function closeEditSessionModal() {
    ...
    if (editSessionReturnToList) {
        editSessionReturnToList = false;
        goTableList();
    }
}
...
function switchPage(pageNum, doSave = true) {
    ...
    const t = getActiveTable();
    if (t) applySessionModeUI(getEffectiveMode(t));
...
// 稼働タイプに応じてPage1/Page3のUIを切り替える
// 'new'(新規/稼働用): 後ヅモ関連の項目を隠し、タブ分けせず1画面で完結させる
// 'mid'(途中/後ヅモ用): 開始G数・前任者情報・台全体⇄自分の区間タブを表示
function applySessionModeUI(mode) {
    const isMid = mode === 'mid';

    // ---- Page1 ----
    const page1TabBar = document.getElementById('page1SubTabBar');
    const startGamesBox = document.getElementById('startGamesBox');
    const prevCountsCard = document.getElementById('prevCountsCard');
    const ownExtraInfoBox = document.getElementById('ownExtraInfoBox');

    if (page1TabBar) page1TabBar.style.display = isMid ? 'flex' : 'none';
    if (startGamesBox) startGamesBox.style.display = isMid ? 'flex' : 'none';
    if (prevCountsCard) prevCountsCard.style.display = isMid ? 'block' : 'none';
    if (ownExtraInfoBox) ownExtraInfoBox.style.display = isMid ? 'block' : 'none';

    const page1SubA = document.getElementById('page1SubA');
    const page1SubB = document.getElementById('page1SubB');
    if (isMid) {
        switchPage1Sub(1);
    } else {
        // 新規モードはタブ分けせず、総回転数・BIG/REG/合算・カウンターを1画面で表示
        if (page1SubA) page1SubA.classList.add('active');
        if (page1SubB) page1SubB.classList.add('active');
    }

    // ---- Page3 ----
    const page3TabBar = document.getElementById('page3SubTabBar');
    if (page3TabBar) page3TabBar.style.display = isMid ? 'flex' : 'none';

    const page3SubA = document.getElementById('page3SubA');
    const page3SubB = document.getElementById('page3SubB');
    const page3SubBTitle = document.getElementById('page3SubBTitle');
    const page3RefOwnTitle = document.getElementById('page3RefOwnTitle');
    const page3OwnNote = document.getElementById('page3OwnNote');
    const ownSegmentInfoPage3El = document.getElementById('ownSegmentInfoPage3');

    if (isMid) {
        switchPage3Sub(1);
        if (page3SubBTitle) page3SubBTitle.innerText = '【 自分の区間：全項目の近似値判定 】';
        if (page3RefOwnTitle) page3RefOwnTitle.innerText = '【 自分の区間：設定スペックリファレンス（全項目） 】';
        if (page3OwnNote) page3OwnNote.style.display = 'block';
        if (ownSegmentInfoPage3El) ownSegmentInfoPage3El.style.display = 'block';
    } else {
        // 新規モードは「台全体」タブ（合算/BIG/REGのみ）を隠し、全項目タブだけを単独表示
        if (page3SubA) page3SubA.classList.remove('active');
        if (page3SubB) page3SubB.classList.add('active');
        if (page3SubBTitle) page3SubBTitle.innerText = '【 現在の細分化・近似値判定 】';
        if (page3RefOwnTitle) page3RefOwnTitle.innerText = '【 全設定スペックリファレンス 】';
        if (page3OwnNote) page3OwnNote.style.display = 'none';
        if (ownSegmentInfoPage3El) ownSegmentInfoPage3El.style.display = 'none';
    }

    // ---- Page4 ----
    const sumGamesLabel = document.getElementById('sumGamesLabel');
    if (sumGamesLabel) sumGamesLabel.innerText = isMid ? '総回転数(自分の区間)' : '総回転数';
}
...
function registerBonus() {
    ...
    updatePage2History();
    switchPage(1);
    // 途中(後ヅモ)モードでは①全体情報ではなく②自分の区間(カウンター)タブに戻す
    // 新規モードはタブ分けがなく両方常に表示されているため、切り替え操作自体が不要
    if (getEffectiveMode(t) === 'mid') {
        switchPage1Sub(2);
    }
}
...
function updateApproximationTable() {
    ...
    // リファレンス表のヘッダー行（共通で使い回す）
    const buildRefHeader = () => {
        let h = `<tr><th>項目</th>`;
        for (let s = 1; s <= 6; s++) {
            h += `<th style="${getSettingColorClass(s)}">設定${s}</th>`;
        }
        h += `</tr>`;
        return h;
    };
    const buildRefRowsHtml = (rows) => {
        let h = '';
        rows.forEach(row => {
            h += `<tr><td>${row[0]}</td>`;
            for (let s = 0; s < 6; s++) {
                h += `<td style="${getSettingColorClass(s+1)}">1/${row[1][s]}</td>`;
            }
            h += `</tr>`;
        });
        return h;
    ...
    // ① 台全体タブ用リファレンス（合算・BIG・REGのみ）
    const refRowsSummary = [
        ['ボーナス合算', spec.synth],
        ['BIG確率', spec.big],
        ['REG確率', spec.reg]
    ];
    document.getElementById('refTable').innerHTML = buildRefHeader() + buildRefRowsHtml(refRowsSummary);
    ...
    document.getElementById('refTableOwn').innerHTML = buildRefHeader() + buildRefRowsHtml(refRowsFull);
...
function updateDetailedSummary() {
    ...
    // 収支・機械割は「自分の投資/回収」に対応させるため、自分の区間の回転数を使う
    const g = computeOwnGames(t);
    ...
    // トータル合算は自分が追加したボーナスと自分の区間の回転数を反映

...
function updateDisplay() {
    ...
    const budouVal = (t.budou > 0 && g > 0) ? g / t.budou : 0;
    const cherryVal = (t.cherry > 0 && g > 0) ? g / t.cherry : 0;
...
function updateApproximationTable() {
    const t = getActiveTable();
    if (!t) return;
    const table = document.getElementById('approxTable');
    ...
    const g = t.games;
    if (g <= 0) {
        table.innerHTML = `<tr><th>項目</th><th>現在値</th><th>近似設定</th></tr><tr><td colspan="3" style="text-align:center; color:var(--muted-color); padding:15px;">総回転数を入力してください</td></tr>`;
        return;
    }
    ...
    const budouVal = t.budou > 0 ? g / t.budou : 0;
    const cherryVal = t.cherry > 0 ? g / t.cherry : 0;

    // 細分化判定からは「レアチェリー / 一枚役」「不明」を除外し、明示的な単独・チェリー重複のみ対象とする
    const singleBigCount = t.history ? t.history.filter(h => h.type === 'BIG' && h.trigger.startsWith('単独')).length : 0;
    const singleBigVal = singleBigCount > 0 ? g / singleBigCount : 0;

    const singleRegCount = t.history ? t.history.filter(h => h.type === 'REG' && h.trigger.startsWith('単独')).length : 0;
    const singleRegVal = singleRegCount > 0 ? g / singleRegCount : 0;

    const cherryBigCount = t.history ? t.history.filter(h => h.type === 'BIG' && h.trigger.startsWith('チェリー重複')).length : 0;
    const cherryBigVal = cherryBigCount > 0 ? g / cherryBigCount : 0;

    const cherryRegCount = t.history ? t.history.filter(h => h.type === 'REG' && h.trigger.startsWith('チェリー重複')).length : 0;
    ...
    const rowsData = [
        ['ボーナス合算', gappei > 0 ? `1/${gappei.toFixed(1)}` : '-', getBestSettingNum(gappei, spec.synth, gappei === 0)],
        ['ぶどう確率', budouVal > 0 ? `1/${budouVal.toFixed(2)}` : '-', getBestSettingNum(budouVal, spec.budou, budouVal === 0)],
        ['チェリー確率', cherryVal > 0 ? `1/${cherryVal.toFixed(1)}` : '-', getBestSettingNum(cherryVal, spec.cherry, cherryVal === 0)],
        ['単独BIG', singleBigVal > 0 ? `1/${singleBigVal.toFixed(1)}` : '未検出', getBestSettingNum(singleBigVal, spec.singleBig, singleBigVal === 0)],
        ['単独REG', singleRegVal > 0 ? `1/${singleRegVal.toFixed(1)}` : '未検出', getBestSettingNum(singleRegVal, spec.singleReg, singleRegVal === 0)],
        ['チェリー重複BIG', cherryBigVal > 0 ? `1/${cherryBigVal.toFixed(1)}` : '未検出', getBestSettingNum(cherryBigVal, spec.cherryBig, cherryBigVal === 0)],
        ['チェリー重複REG', cherryRegVal > 0 ? `1/${cherryRegVal.toFixed(1)}` : '未検出', getBestSettingNum(cherryRegVal, spec.cherryReg, cherryRegVal === 0)]
    ];
